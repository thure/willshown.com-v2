  ...
  playIntroAnimation(){
    ...
    anime.timeline().add({
      ...
      update: function(anim){
        ...
        $animRoot.style.setProperty('box-shadow', `0 ${p * 19}px ${p * 38}px rgba(0,0,0,0.30), 0 ${p * 15}px ${p * 12}px rgba(0,0,0,0.22)`)
      },
    }).add({
      targets: $animRoot,
      duration: 400,
      ...
      update: function(anim){
        if(anim.currentTime > anim.offset){
          const p = anime.easings.easeInOutExpo((anim.currentTime - anim.offset) / (anim.duration - anim.offset))
          $animRoot.style.setProperty('box-shadow', `0 ${19 - p * (19 - 3)}px ${38 - p * (38 - 6)}px rgba(0,0,0,${.30 - p * (.30 - .16)}), 0 ${15 - p * (15 - 3)}px ${12 - p * (12 - 6)}px rgba(0,0,0,${.22 - p * (.22 - .23)})`)
        }
      }
    })
    ...
    anime.timeline().add({
      ...
      duration: 2000,
      strokeDasharray: {
        value: [()=>`0 ${anime.setDashoffset($markPaths[0]) / 5}`, ()=>`${anime.setDashoffset($markPaths[0]) / 5}, 0`],
        easing: 'easeInOutExpo'
      },
      strokeWidth: {
        value: [0, 2],
